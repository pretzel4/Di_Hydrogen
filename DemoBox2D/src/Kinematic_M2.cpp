...
Kinematic_M2::Kinematic_M2() {
	
}


b2Fixture* Kinematic_M2::setup_body(RectF &rect, b2BodyDef *overwrite_body, b2Shape *overwrite_shape, b2FixtureDef *fixture_def) {
...
void Kinematic_M2::setup_body_adv(std::vector<Vector2> &vertices) {
	b2BodyDef bodydef;
	bodydef.type = b2_kinematicBody;
	body = world_ptr->getb2World()->CreateBody(&bodydef);
