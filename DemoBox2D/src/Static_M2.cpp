...
b2Fixture* Static_M2::setup_body(RectF &rect, b2BodyDef *overwrite_body, b2Shape *overwrite_shape, b2FixtureDef *fixture_def) {
...
void Static_M2::setup_body_adv(std::vector<Vector2> &vertices) {
	b2BodyDef bodydef;
	bodydef.type = b2_staticBody;
	body = world_ptr->getb2World()->CreateBody(&bodydef);
